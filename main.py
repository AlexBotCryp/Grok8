...
TAKE_PROFIT = 0.03     # Reducido a 3% para más operaciones en mercados estables
STOP_LOSS = -0.03      # Ajustado a -3% para permitir más movimientos
...
MAX_POSICIONES = 7     # Mantener en 7 para manejar posiciones
    ...
    try:
        ...
        for cripto in criptos:
            ...
            try:
                ...
                prompt = f"Analiza {symbol}: Precio {precio:.4f}, Cambio {change_percent:.2f}%, Volumen {volume:.2f}, RSI {rsi:.2f}. ¿Comprar con {cantidad_usdc:.2f} USDC? Supervisa solo, prioriza RSI < 45. Responde solo con 'sí' o 'no'."
                ...
                ganancia_neta = ganancia_bruta - (comision_compra + comision_venta)
                if (rsi < 45 or change_percent > 0.5) and ganancia_neta > 0 and ('sí' in grok_response or consulta_contador % GROK_CONSULTA_FRECUENCIA != 0):
...
def vender_y_convertir():
    ...
    for symbol, data in list(registro.items()):
        try:
            ...
            closes = [float(k[4]) for k in klines]
            rsi = calculate_rsi(closes)
            prompt = f"Para {symbol}: Precio compra {precio_compra:.4f}, actual {precio_actual:.4f}, cambio {cambio*100:.2f}%, RSI {rsi:.2f}. ¿Vender ahora para convertir? Supervisa solo, prioriza RSI > 65 o ganancia >3%. Responde solo con 'sí' o 'no'."
            grok_response = consultar_grok(prompt)
            ...
            vender_por_profit = (cambio >= TAKE_PROFIT or rsi > 65 or 'sí' in grok_response) and ganancia_neta > 0
