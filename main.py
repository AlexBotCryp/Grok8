...
FORCE_BUY_SYMBOL = env("FORCE_BUY_SYMBOL", "").strip().upper()   # p.ej. BTCUSDC (o vacío para desactivar)
...
def evaluate_and_trade():
    ...
    for sym in SYMBOLS:
        ...
        try:
            ...
            if DEBUG: print(f"[BAL] USDC libre ≈ {usdc_bal:.2f}", flush=True)
            ...
            if be.code == -1003:
                tg_send("⛔️ IP baneada por peso REST. Usamos WS; espera a que se levante.")
            else:
...
def main():
    ...
    # Compra forzada opcional (para validar órdenes/Telegram/estado)
